# Wintors & Caaines — Departamento de Coleção (6 robôs)
...
| 2 | Varredura de Concorrentes | `concorrentes/` | `/colecao-concorrentes` | Bimestral |
...
| 6 | Coleção — decide, audita e opina | `colecao/` | `/colecao-decisao` | Por ciclo de coleção |
...
`Robô 4 (mix/vendas)` roda **antes** do `Robô 6 (coleção)` — sem histórico, não se propõe peça. Os robôs 1-3 e 5 alimentam quando têm output fresco; o Robô 6 declara a idade de cada insumo que usou.
...
| Chrome conectado (Pinterest logado, captura de concorrente) | ✅ funciona no PC | Robôs 2, 3 |
